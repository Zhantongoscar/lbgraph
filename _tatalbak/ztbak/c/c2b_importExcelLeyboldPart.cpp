...
int main() {
    ...
    // 执行查询：筛选和排序特定类型的设备
    cout << "\n正在执行筛选查询并删除不符合条件的记录..." << endl;
    
    // 首先创建一个临时表来存储符合条件的记录
    string create_temp_table = R"(
CREATE TEMPORARY TABLE temp_filtered_records AS
SELECT t1.*
FROM leybold_device_lib t1
JOIN (
    -- 对于每个bom_class和operating_element组合，找到最小的ID
    SELECT MIN(id) as min_id
    FROM leybold_device_lib
    WHERE bom_class LIKE 'K1.%' 
    AND (operating_element LIKE 'K%' OR operating_element LIKE 'Q%' OR operating_element LIKE 'S%')
    GROUP BY bom_class, operating_element
...
ORDER BY t1.bom_class ASC, t1.operating_element ASC;
    )";

    if (mysql_query(conn, create_temp_table.c_str())) {
        cerr << "创建临时表失败: " << mysql_error(conn) << endl;
    } else {
        cout << "临时表创建成功" << endl;
        
        // 查询符合条件的记录总数
        string count_query = "SELECT COUNT(*) FROM temp_filtered_records;";
        if (mysql_query(conn, count_query.c_str())) {
            cerr << "查询临时表记录数失败: " << mysql_error(conn) << endl;
        } else {
            MYSQL_RES* count_result = mysql_store_result(conn);
            MYSQL_ROW count_row = mysql_fetch_row(count_result);
            int filtered_count = count_row ? atoi(count_row[0]) : 0;
            mysql_free_result(count_result);
            
            cout << "符合条件的记录数: " << filtered_count << endl;
            
            // 删除不符合条件的记录
            string delete_query = R"(
DELETE FROM leybold_device_lib
WHERE id NOT IN (SELECT id FROM temp_filtered_records);
            )";
            
            if (mysql_query(conn, delete_query.c_str())) {
                cerr << "删除不符合条件的记录失败: " << mysql_error(conn) << endl;
            } else {
                int affected_rows = mysql_affected_rows(conn);
                cout << "已删除 " << affected_rows << " 条不符合条件的记录" << endl;
                
                // 显示保留下来的记录
                string select_query = "SELECT * FROM leybold_device_lib ORDER BY bom_class ASC, operating_element ASC;";
                if (mysql_query(conn, select_query.c_str())) {
                    cerr << "查询结果失败: " << mysql_error(conn) << endl;
                } else {
