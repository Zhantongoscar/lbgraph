...
private:
    ...
    bool createConnectionsInNeo4j() {
        ...
        scriptFile << "        # 检查并打印一些节点示例\n";
        scriptFile << "        print('\\n检查节点属性示例:')\n";
        scriptFile << "        result = session.run(\n";
        scriptFile << "            '''\n";
        scriptFile << "            MATCH (n) \n";
        scriptFile << "            WHERE n:V_Device OR n:V_terminal \n";
        scriptFile << "            RETURN n, labels(n) AS labels\n";
        scriptFile << "            LIMIT 5\n";
        scriptFile << "            '''\n";
        scriptFile << "        )\n";
        scriptFile << "        for record in result:\n";
        scriptFile << "            node = record['n']\n";
        scriptFile << "            labels = record['labels']\n";
        scriptFile << "            print(f'\\n节点类型: {labels}')\n";
        scriptFile << "            for key, value in node.items():\n";
        scriptFile << "                print(f'  {key}: {value}')\n\n";
        ...
        scriptFile << "            MATCH (n)\n";
        scriptFile << "            WHERE n:V_Device OR n:V_terminal\n";
        scriptFile << "            RETURN n.FTID AS ftid, n.fdid AS fdid\n";
        scriptFile << "            '''\n";
        scriptFile << "        )\n";
        scriptFile << "        nodes_by_ftid = {record['ftid']: True for record in result if record['ftid']}\n";
        scriptFile << "        print(f'从Neo4j读取了 {len(nodes_by_ftid)} 个节点的FTID')\n\n";
        ...
        scriptFile << "            \n";
        scriptFile << "            # 确保source和target有等号前缀\n";
        scriptFile << "            if not source.startswith('='):\n";
        scriptFile << "                source = '=' + source\n";
        scriptFile << "            if not target.startswith('='):\n";
        scriptFile << "                target = '=' + target\n";
        scriptFile << "            \n";
        scriptFile << "            if source not in nodes_by_ftid or target not in nodes_by_ftid:\n";
        scriptFile << "                if source not in nodes_by_ftid:\n";
        scriptFile << "                    missing_nodes.add(source)\n";
        scriptFile << "                if target not in nodes_by_ftid:\n";
        scriptFile << "                    missing_nodes.add(target)\n";
        scriptFile << "                print(f'未能找到节点: source={source}, target={target}')\n";
        ...
        scriptFile << "                    WHERE source.FTID = $source\n";
        scriptFile << "                    MATCH (target)\n";
        scriptFile << "                    WHERE target.FTID = $target\n";
        ...
        scriptFile << "    if missing_nodes:\n";
        scriptFile << "        print('\\n缺失的节点:')\n";
        scriptFile << "        for node in sorted(missing_nodes):\n";
        scriptFile << "            print(f'  - {node}')\n\n";
